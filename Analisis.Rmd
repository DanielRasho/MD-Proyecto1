...
El drama es el genero principal con 3789 ocurrencias. Lo cual también coincide con las peliculas más recientes que tienen que ver con drama.

```{r}
movies %>% 
  select(title, releaseDate, genres) %>%  # Select only 'title' and 'releaseDate' columns
  arrange(desc(releaseDate)) %>%  # Order by 'releaseDate' in descending order
  head(n = 20)  # Display the first few rows
```
...
# Order by Frequency in descending order
freq_table <- genres_freq %>% arrange(desc(Frequency))

freq_table
    
ggplot(freq_table, aes(x = reorder(Genres, Frequency), y = Frequency, fill = genres)) +
  geom_bar(stat = "identity") +
  labs(title = "Frecuencia de generos",
       x = "Géneros",
       y = "Frecuencia") +
  theme_minimal() +
...
Al revisar la relación entre ambos sets de datos, se puede indicar que no hay relación entre las variables al, al tener un coeficiente de correlación de Pearson del -2%, es decir, aproximadamente la cantidad de actores solo influye en un 2% y los ingresos generados. 
```{r}

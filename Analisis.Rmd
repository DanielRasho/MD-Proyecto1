...
most_profitable_genre <- movies %>%
  separate_rows(genres, sep = "\\|") %>%  # Splitting multiple genres
  mutate(profit = revenue - budget) %>%  # Calculate profit
  group_by(genres) %>%
...
Como se puede evidenciar apartir de la Grafica 1, cuando el rango de presupuesto es mas elevado este suele presentar una mayor cantidad de ingres. Al igual como se ve en la Grafica 2, que en la mayoria de peliculas, se puede evidenciar que tener un presupuesto elevado significa tener mas ingresos. 
...
Se puede observar que hay meses 5 meses con una ganancia promedio superiores a los $50,000,000, estos meses son Mayo, Junio, Julio y Noviembre y Diciembre. Una posible causa es que coinciden con epocas de vacaciones de ciclos educativos.
...
El presupuesto esta fuertemente relacionadod (r^2=0.76) en los ingresos que puede recibir un largometraje. Como se vieron en las medidas anteriores como el promedio de calificaciones o el director influye muy poco (r^2=0.14 y r^2=0.08), esto nos puede sugerir que no es necesario tener estrellas de hollywood o criticos a favor, para tener un película exitosa, como tener medios económicos para hacer algo impactante.
...
En las producción cinematográfica es común comisionar tareas a estudios terceros. La forma de esta gráfica se podría interpretar, en que el número de productoras coincide con mayores ingresos, hasta las 7, a partir de ahí los ingresos tienden a disminuir. Esto podría ser causado a que manejar una comunicación entre tantos equipos puede ser negativo para la producción creativa, además que puede involucrar mayores costos mantener tantos equipos.
...
top_actors <- suppressWarnings(movies %>%
  separate_rows(actors, sep = "\\|") %>%
  filter(
    revenue > median(revenue, na.rm = TRUE),  # Only high-revenue movies
    actors != "FALSE",                        # Remove invalid values
    actors != "",                             # Remove empty actor names (if any)
    !is.na(actors)                            # Remove NA values
  ) %>%
  count(actors, sort = TRUE) %>%
  slice(1))  # Get the top actor
top_actors
```

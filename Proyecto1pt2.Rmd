...
```{r}
movies <- movies %>%
  dplyr::select(-id, -originalTitle, -originalLanguage, -title, -homePage, -video, -productionCompanyCountry, -director, -productionCompany, -genres, -productionCountry, -actorsPopularity, -actors, -actorsCharacter, -releaseDate)

```

```{r discretizar}

movies$popularity <- discretize(movies$popularity, method = "interval", categories = 4)
movies$budget <- discretize(movies$budget, method = "interval", categories = 4)
movies$revenue <- discretize(movies$revenue, method = "interval", categories = 4)
movies$runtime <- discretize(movies$runtime, method = "interval", categories = 4)
movies$voteCount <- discretize(movies$voteCount, method = "interval", categories = 4)
movies$voteAvg <- discretize(movies$voteAvg, method = "interval", categories = 4)
# -----
# Estas variables tienen que ser factor pero aun no lo implemento

#movies$actorsPopularity <- discretize(movies$actorsPopularity, method = "interval", categories = 4)
#movies$actorsAmount <- discretize(movies$actorsAmount, method = "interval", categories = 4)
# ----- 

# Tanto para women como men cuando encuentra un valor NA lo que hace es ponerle la mediana
movies$castWomenAmount <- as.numeric(movies$castWomenAmount)
movies$castMenAmount <- as.numeric(movies$castMenAmount)

# Reemplazar NA con la mediana de la columna
movies$castWomenAmount[is.na(movies$castWomenAmount)] <- median(movies$castWomenAmount, na.rm = TRUE)
movies$castMenAmount[is.na(movies$castMenAmount)] <- median(movies$castMenAmount, na.rm = TRUE)

movies$castWomenAmount <- discretize(movies$castWomenAmount, method = "interval", categories = 4)
movies$castMenAmount <- discretize(movies$castMenAmount, method = "interval", categories = 4)

```

```{r}
movies_matrix <- as(movies, "transactions")
#summary(movies_matrix)

```

## Pruebas de diferentes valores de soporte y confianza

```{r}
rules <- apriori(movies_matrix, parameter = list(supp = 0.01, conf = 0.5, minlen = 2))
rules_sorted <- sort(rules, by="confidence", decreasing=TRUE)
rules_top <- rules_sorted[1:10] 
inspect(rules_top)
...
rules <- apriori(movies_matrix, parameter = list(supp = 0.01, conf = 0.6, minlen = 2))
length(rules)

rules <- apriori(movies_matrix, parameter = list(supp = 0.02, conf = 0.6, minlen = 2))
rules_sorted <- sort(rules, by="confidence", decreasing=TRUE)
rules_top <- rules_sorted[1:10] 
inspect(rules_top)
```
## Reglas con los mejor valores encontrados
```{r}
rules <- apriori(movies_matrix, parameter = list(supp = 0.05, conf = 0.6, minlen = 2))
rules_sorted <- sort(rules, by="confidence", decreasing=TRUE)
rules_top <- rules_sorted[1:10] 
inspect(rules_top)
```
...
Lo que se puede interpretar de estas reglas (de la 1 a las 4) es que en las peliculas con calificaciones bajas (entre 3.47 y 5.65) tienden a tener un numero bajo de votos, generan menos ingresos, tienen una duracion moderada y a ser menos pupulares. Ademas, en las reglas (5 y 6) Las peliculas con 0 a 2 generos suelen tener menor recaudacion y menos popularidad. Reglas (7-8) las peliculas con mas actores tienden a tener la misma cantidad de actores masculinos como femeninos y las peliculas con menos actores suelen producirse en menos paises.
